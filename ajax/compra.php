...
switch ($_GET["op"]) {
    ...
    case 'listarArticulos':
        ...
        $rspta = $articulo->listarActivos(1);
        $data = array();
        ...
        while ($reg = $rspta->fetch_object()) {
            ...
            $data[] = array(
                "0" => $reg->codigo,
                "1" => $reg->nombre,
                "2" => $reg->partida_generica,
                "3" => $reg->nombreum,
                "4" => $reg->stock,
                "5" => $reg->precio_final_kardex,
                "6" => '<button class="btn btn-primary btn-sm btn-wave waves-effect waves-light" onclick="agregarDetalle(' . $reg->idarticulo . ',\'' . $reg->nombre . '\',\'' . $reg->partida_generica . '\',\'' . $reg->codigo . '\',\'' . $reg->nombre . '\',\'' . /*$reg->precio_venta*/"" . '\',\'' . $reg->stock . '\',\'' . $reg->abre . '\' ,\'' . /*$reg->precio_unitario*/"" . '\', \'' . $reg->costo_compra . '\', \'' . /*$reg->factorc*/"" . '\' , \'' . $reg->nombreum . '\')">Agregar</button>'
            );
