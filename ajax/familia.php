...
switch ($_GET["op"]) {
	...
	case 'listar':
		...
		while ($reg = $rspta->fetch_object()) {
			$data[] = array(
				"0" => ($reg->estado) ? '<button class="btn btn-icon btn-wave waves-effect waves-light btn-sm btn-info-light" onclick="mostrar(' . $reg->idfamilia . ')"><i class="ri-edit-line"></i></button>' .
					' <button class="btn btn-icon btn-wave waves-effect waves-light btn-sm btn-danger-light" onclick="desactivar(' . $reg->idfamilia . ')"><i class="ri-delete-bin-line"></i></button>' :
					'<button class="btn btn-icon btn-wave waves-effect waves-light btn-sm btn-info-light" onclick="mostrar(' . $reg->idfamilia . ')"><i class="ri-edit-line"></i></button>' .
					' <button class="btn btn-icon btn-wave waves-effect waves-light btn-sm btn-success-light" onclick="activar(' . $reg->idfamilia . ')"><i class="ri-check-double-line"></i></button>',
				"1" => $reg->descripcion,
