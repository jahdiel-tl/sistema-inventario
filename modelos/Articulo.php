...
class Articulo
{
            ...
            )
    {
        $sql = "insert into
         articulo 
         (
            idarticulo,                                         idfamilia,
            partida_generica,                                   nombre,
            unidad_medida,                                      marca,
            modelo,                                             codigo,
            dimensiones,                                        color,
            caracteristicas,                                    costo_compra,
            estado,                                             portador,
            idalmacen,                                          stock,
            ruc,                                                proveedor,
            seriefaccompra,                                     numerofaccompra,
            fechafacturacompra,                                 umedidacompra,
            imagen,                                             costo_inicial,
            fecharegistro
         )
    ...
    public function articuloBusqueda($codigo)
    {
        ...
        $sql = "select nombre, stock, costo_compra, um.abre  as  unidad_medida from articulo a inner join umedida um on a.unidad_medida=um.idunidad   where codigo='$codigo'";
    ...
    public function listar($idempresa)
    {
        ...
        $sql = "
                select 
                    ...
                    articulo a inner join familia f on a.idfamilia=f.idfamilia inner join almacen al on a.idalmacen=al.idalmacen inner join institucion e on al.idempresa=e.idinstitucion inner join umedida um on a.umedidacompra=um.idunidad
                     where 
                     not a.nombre='1000ncdg' and e.idinstitucion='$idempresa' and al.estado='1'";
    ...
    public function listarActivos($idempresa)
    {
        ...
        from 
        articulo a inner join familia f ON a.idfamilia=f.idfamilia  inner join  almacen al on a.idalmacen=al.idalmacen inner join  institucion e on al.idempresa=e.idinstitucion  inner join umedida um on a.umedidacompra=um.idunidad
        where a.estado='1' and not a.nombre ='1000ncdg' and e.idinstitucion='$idempresa' and al.estado='1'";
        return ejecutarConsulta($sql);
    ...
    public function listarActivossubarticulo($idempresa)
    {
        ...
        um.abre,
        um.nombreum, 
        (a.costo_compra * 1.18) as precio_unitario, 
        a.precio_final_kardex
        from 
        articulo a inner join familia f ON a.idfamilia=f.idfamilia  inner join  almacen al on a.idalmacen=al.idalmacen inner join  institucion e on al.idempresa=e.idinstitucion  inner join umedida um on a.umedidacompra=um.idunidad
        where a.estado='1' and not a.nombre ='1000ncdg' and e.idinstitucion='$idempresa' and al.estado='1'";
...
select  valor_venta_item_32 as tventas from detalle_boleta_producto db inner join articulo a on db.idarticulo=a.idarticulo
    ...
    public function totalcomprasgeneral($ano, $mes)
    {
        ...
        inner join compra c on dc.idcompra=c.idcompra where year(c.fecha)='$ano' and month(c.fecha)='$mes' order by a.idarticulo asc  ";
        return ejecutarConsulta($sql);
    }
    ...
    public function totalventasgeneral($ano, $mes)
    {
        ...
        inner join boleta b 
            on db.idboleta=b.idboleta where year(b.fecha_emision_01)='$ano' and month(b.fecha_emision_01)='$mes' 
        union all
        ...
        ) as tabla order by idarticulo asc ";
        return ejecutarConsulta($sql);
    }
