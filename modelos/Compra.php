...
// (COSTO DE COMPRA * CANTIDAD)+VALOR FINAL KARDEX / (SALDO FINAL + CANTIDAD)
            ...
            entradas=entradas + '$cantidad[$num_elementos]', 
            valor_finu=((saldo_iniu + entradas) - salidas) * precio_final_kardex, 
            stock=saldo_finu
            where idarticulo='$idarticulo[$num_elementos]'";
...
// (COSTO DE COMPRA * CANTIDAD)+VALOR FINAL KARDEX / (SALDO FINAL + CANTIDAD)
    ...
    public function listar($idempresa)
    {
        ...
        c.estado 
        from 
        compra c inner join persona p on c.idproveedor=p.idpersona inner join usuario u on c.idusuario=u.idusuario inner join catalogo1 ct1 on c.tipo_documento=ct1.codigo inner join  institucion e on c.idinstitucion=e.idinstitucion where e.idinstitucion='$idempresa'  order by c.idcompra desc ";
        return ejecutarConsulta($sql);      
    }
