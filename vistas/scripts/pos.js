...
function refrescartabla_destinatario()
{
    tabla_destinatario.ajax.reload();
}

function listarDestinatario()
{
    tabla_destinatario=$('#tbladestinatario').dataTable(
    {
        ...
        "ajax":
                {
                    url: '../ajax/guiaremision.php?op=listarDestinatario&tipo_persona=cliente',
                    type : "get",
                    dataType : "json",                      
                    error: function(e){
...
$('#myModalDestinatario').on('hidden.bs.modal', function () {
});


function agregarDestinatario(idpersona, numero_documento, razon_social, departamento, ciudad, distrito, domicilio_fiscal)
  {
    ...
    if (idpersona!="")
    {
        ...
        //$('#myModalDestinatario').modal('hide');
    }
    else
    {
        alert("Error , revisar los datos del Destinatario");
    }
  ...
  if (data.ListaProductos && data.ListaProductos.length > 0) {
    data.ListaProductos.forEach(product => 
    {
      ...
      var card = `
        ...
        <span class="d-flex align-items-center text-muted mt-auto ms-2" style="font-size: 14px;"> costo: S/ <input id="p_costo_compra" class="text-muted" readonly value="${product.costo_compra}" style="border-radius: 10px;width: 48%;height: 15px;border: none;font-size: 14px;pointer-events: none;user-select: none;"></span>
...
/* ---------------------------------------------------------------- */
//              FUNCION AGREGAR PRODUCTO POR CODIGO BARRA
...
function agregarProductPedido(
  ...
  if (tipocomprobante == 0) {
    ...
    if (productPrice == 0) {
        ...
        swal.fire({
          title: "Error",
          text: 'Este producto no se puede agregar porque el precio es S/ 0.00',
          icon: "error",
          timer: 2000,
          showConfirmButton: false
        });
          ...
          <div class="w-100">
            <div class="d-flex justify-content-between align-items-center">
              ...
              <div class="quantity rounded-pill d-flex justify-content-center align-items-center">
                <!-- <button class="btn btn-sm btn-warning rounded-circle minus" id="ped_disminuir" aria-label="Decrease">&minus  &plus ;</button> -->
                <input type="number" class="input-box" name="cantidad_item_12[]" id="ped_cantidad" value="1" min="1" max="${productStock}">
                <button class="btn btn-sm btn-success rounded-circle plus" id="ped_aumentar" aria-label="Increase">&#x2713;</button>
              </div>
            </div>
            <div class="d-flex justify-content-between align-items-baseline"> `;

            //if(productName!="FLETE DE TRANSPORTE")
            //{
            //  newItem +=`<span>S/  <input type="number" class="border-0" name="precio_unitario[]" id="precio_unitario[]" value="${productPrice}" onBlur="modificarSubtotales(1)" style="background: transparent;" readonly></span>`;
            //}
            //else
            //{
              newItem +=`<span>S/  <input type="number" class="border-0" name="precio_unitario[]" id="precio_unitario[]" value="${productPrice}" onBlur="modificarSubtotales(1)" style="background: transparent;"></span>`;
            //}
...
//Bloquear controlador de div buton metodopago
$(document).on('click', '.quantity .input-box', function (e) {
    e.preventDefault();
    var _btn_metodopago = document.getElementById('btn_metodopago');
    _btn_metodopago.disabled = true;
    ...
    
  {
    ...
    for (var i = 0; i < cant.length; i++) {
      ...
      if (inpC.value == '') {
        ...
        document.getElementsByName("cantidad_item_12[]")[i].value = 0;

      } else if (isNaN(inpC.value) || inpC.value < 0) {
        // Si el valor no es un número válido o está vacío, muestra una alerta
        swal.fire({
          title: "Ops..!",
          text: "La cantidad ingresada no es válido",
          icon: "warning",
          timer: 2000,
          showConfirmButton: false
        });

        document.getElementsByName("cantidad_item_12[]")[i].focus();
      }
    }
...
function increaseValue(input, inputBox, inputcant) {
  ...
  var _btn_metodopago = document.getElementById('btn_metodopago');
  _btn_metodopago.disabled = false;
  handleQuantityChange(input, inputBox, inputcant);
...
var rutaMandarWhatsComprobante = ''
const mandarWhatsComprobante = () =>{
  var telefono = document.querySelector('#whatsTelefono').value
  if(telefono === '' || telefono === null) {
    Swal.fire({
      title: 'Valor Requerido',
      text: 'Por favor, ingrese un telefono antes de continuar.',
      icon: 'warning',
      confirmButtonText: 'OK'
    });
    return
  }
  var whatsappURL = 'https://api.whatsapp.com/send?phone=' + encodeURIComponent(telefono) + '&text=' + encodeURIComponent('¡Hola! Te envío este comprobante: ' + rutaMandarWhatsComprobante.replace('..', 'https://factura.serviceappweb.com'));
    ...
    $.post(
      ...
      function (data, status) {
        ...
        if (data.tipoimpresion == "00") {
          ...
          rutaMandarWhatsComprobante = rutacarpeta
